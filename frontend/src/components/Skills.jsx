      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-16">
          {skillCategories.map((category, idx) => (
            ...
            >
              <div className="absolute -inset-8 bg-jewel-emerald/5 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />

              <h3 className="text-zinc-500 font-sans text-[10px] uppercase tracking-[0.2em] mb-8 group-hover:text-jewel-emerald-deep transition-colors relative z-10">
                {category.title}
              </h3>
              <ul className="space-y-6 relative z-10">
                {category.skills.map((skill) => (
                  <li key={skill} className="flex items-center group/item">
                    ...
                    <span className="text-3xl md:text-4xl font-serif italic text-zinc-500 group-hover/item:text-jewel-emerald transition-colors cursor-default">
                      {skill}
                    </span>
                  </li>
                ))}
